...
message_str_add_watch = ("Start monitoring new transactions for address %s - "
                         "<a href='https://blocksmurfer.io/search/%s'>%s</a>")
message_str_new_tx_found = "New transactions for address <a href='https://blocksmurfer.io/search/%s'>%s</a> with txid %s"
debug = True
...
def check_address(address, network, address_name):
    ...
    if latest_txid is None and status_last_txid is None:
        sendmessage(message_str_add_watch % (address_name, address, address))
        file_write_status(monitor_filename, "no-txs")
    if status_last_txid != latest_txid and latest_txid:
        sendmessage(message_str_new_tx_found % (address, address_name, latest_txid))
